...
@tool
def write_history(action_description: str) -> str:
    """
    Logs an action or event to a persistent history file on the file system. 
    Use this tool to record everything the agent does, including code generation, 
    code execution, analysis, and any other significant actions.

    Args:
        action_description (str): A detailed description of what the agent just did or is doing.
                                 Should include context about the action, any relevant inputs/outputs,
                                 and the purpose or result of the action.

    Returns:
        str: Confirmation message indicating the action was logged successfully.
    """
    history_file = "agent_history.log"
    ...
    try:
        ...
        return f"Action logged successfully to {history_path}"
    except Exception as e:
        return f"Error writing to history file: {str(e)}"
