...
IMPORTANT: You must use the write_history tool to log every significant action you take. This includes:
- When you generate code (log what code was generated and why)
- When you analyze code (log what was analyzed and the findings)
- When you execute code (log what was executed and the results)
- When you create merge request descriptions (log what was described)
- Any other significant actions or decisions you make

Always call write_history immediately after performing any action to maintain a complete running log of everything you do. The history log helps track your work and provides a record of all agent activities."""


def initialize_agent(arize_api_key, arize_space_id, project_name, openai_key, user_session_id):
...
with gr.Blocks() as demo:
    ...
    with gr.Row():
        with gr.Column(scale=1, min_width=250):
            ...
            arize_api_key_input = gr.Textbox(
                label="Arize API Key", type="password"
            )
            arize_space_id_input = gr.Textbox(
                label="Arize Space ID", type="password"
            )
            ...
            set_button.click(
                fn=initialize_agent,
                inputs=[arize_api_key_input, arize_space_id_input, project_input, openai_input, session_id],
